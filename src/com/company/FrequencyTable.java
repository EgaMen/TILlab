...
public final class FrequencyTable {
    ...
    public HuffmanTree buildCodeTree() {
        ...
        while (pqueue.size() > 1) {
            ...
            pqueue.add(new NodeWithFrequency(
                    new InternalKnot(x.knot, y.knot),
                    Math.min(x.lowestSymbol, y.lowestSymbol),
                    x.frequency + y.frequency));
        }
        return new HuffmanTree((InternalKnot)pqueue.remove().knot, frequencies.length);
    }
    private static class NodeWithFrequency implements Comparable<NodeWithFrequency> {
        ...
        public NodeWithFrequency(Knot nd, int lowSym, long freq) {
            knot = nd;
            lowestSymbol = lowSym;
            frequency = freq;
        }
